...
"""
ADW Plan, Build & Test — tac_practise

Usage: uv run adw_plan_build_test.py <github-issue-number> [adw-id]

Extends adw_plan_build.py with a test/validation phase that implements
the closed-loop feedback pattern: Request → Validate → Resolve.
...
6. Test Agent: Run validation commands, resolve failures, repeat until green
7. Create PR with full context

The test phase runs all validation commands and loops until everything passes.
IMPORTANT: If any validation step fails, the agent resolves the error and
reruns ALL validation steps from the top.
...
def run_validation(adw_id: str, logger: logging.Logger) -> Tuple[bool, str]:
    """Run all validation commands and return (all_passed, output_summary).

    Runs: lint, build, and tests. Returns the combined output.
    """
    validation_commands = [
        ("Lint", "npm run lint"),
...
def test_and_resolve(adw_id: str, logger: logging.Logger) -> bool:
    """Run the feedback loop: validate → resolve → revalidate until green.

    Returns True if all validations pass within MAX_VALIDATION_RETRIES attempts.
    """
    for attempt in range(1, MAX_VALIDATION_RETRIES + 1):
        logger.info(f"Validation attempt {attempt}/{MAX_VALIDATION_RETRIES}")
        all_passed, summary = run_validation(adw_id, logger)
...
def main():
    """Main ADW orchestrator — Plan, Build, Test."""
    load_dotenv()
    ...
    make_issue_comment(issue_number, f"{adw_id}_ops: ✅ Starting ADW Plan-Build-Test workflow", repo_path)
    ...
    plan_prompt = (
        f"Read the file .github/prompts/{issue_command.replace('/', '')}.prompt.md and follow its instructions "
        f"with these arguments: {issue['title']}: {issue.get('body', '')}\n\n"
        "IMPORTANT: Include a Validation Commands section in the plan. "
        "Every plan must end with commands that prove the work is correct."
    )
    run_claude(plan_prompt, adw_id, logger)
    ...
    implement_prompt = (
        "Read the file .github/prompts/implement.prompt.md and follow its instructions. "
        "Find the latest plan in specs/ and implement it.\n\n"
        "IMPORTANT: After implementing, run all validation commands from the plan. "
        "If you run into any errors at all, stop and resolve them immediately then rerun every validation step."
    )
    run_claude(implement_prompt, adw_id, logger)
    ...
    logger.info("Running validation feedback loop...")
    make_issue_comment(issue_number, f"{adw_id}_tester: ✅ Running validation feedback loop", repo_path)

    validation_passed = test_and_resolve(adw_id, logger)

    if validation_passed:
        make_issue_comment(issue_number, f"{adw_id}_tester: ✅ All validations passed", repo_path)
    else:
        make_issue_comment(
            issue_number,
    ...
    # 9. Commit test fixes (if any)
    git_commit(f"tester: validation complete for #{issue['number']}")
    logger.info("Test phase committed")
    ...
    status = "successfully" if validation_passed else "with validation warnings"
    logger.info(f"ADW Plan-Build-Test workflow completed {status} for issue #{issue_number}")
    make_issue_comment(issue_number, f"{adw_id}_ops: ✅ ADW workflow completed {status}", repo_path)
